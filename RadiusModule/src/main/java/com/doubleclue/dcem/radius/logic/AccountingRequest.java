...
import com.doubleclue.dcem.radius.attribute.IntegerAttribute;
...
public class AccountingRequest extends RadiusPacket {
	...
	public AccountingRequest(String userName, int acctStatusType) {
		super(RadiusPacketType.ACCOUNTING_REQUEST, getNextPacketIdentifier());
	...
	throws RadiusException {
		...
		return ((StringAttribute)ra).getAttributeValue(charset);
	}
	...
	protected byte[] updateRequestAuthenticator(String sharedSecret, int packetLength, byte[] attributes) {
        ...
        md5.reset();
        md5.update((byte)getRadiusPacketType().type);
