...
//	private static SecureRandom random = new SecureRandom();
	 ...
	 */
	public String getAttributeValueHex(String type) {
	...
	public String toString() {
		...
		for (RadiusAttribute attr : attributes) {
			if (attr.getAttributeType() == 2 || attr.getAttributeType() == 3) {  // do never print passwords
				sb.append("\n");
				sb.append(attr.getAttributeType());
				continue;
			}
			sb.append("\n");
	...
	protected byte[] createRequestAuthenticator(String sharedSecret) {
		byte[] secretBytes = sharedSecret.getBytes(DcemConstants.UTF_8);
		byte[] randomBytes = RandomUtils.getRandom(16);
	...
	protected byte[] createResponseAuthenticator(String sharedSecret, int packetLength, byte[] attributes,
		...
		md5.update(sharedSecret.getBytes(DcemConstants.UTF_8));
		return md5.digest();
	}
	...
	public String getNasClient() {
		return nasClient;
	}

	public void setNasClient(String nasClient) {
