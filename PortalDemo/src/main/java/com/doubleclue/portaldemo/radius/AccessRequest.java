...
public class AccessRequest extends RadiusPacket {
	...
	public String getUserName() {
		...
		return ((StringAttribute) attr).getAttributeValue();
	}
	...
	public boolean verifyHash(byte[] pinHash) throws RadiusException {
		if (getAuthProtocol().equals(AUTH_CHAP)) {
			...
			try {
				...
				md.update(new String(getUserPassword().toCharArray()).getBytes("UTF-8"));
				recPinHash = md.digest();
			} catch (Exception e) {
	 ...
	 * 
	 * @see com.doubleclue.portaldemo.radius.tinyradius.packet.RadiusPacket#decodeRequestAttributes(java.lang.String)
	 */
	protected void decodeRequestAttributes(String sharedSecret) throws RadiusException {
		...
		if (userPassword != null) {
			...
			try {
				this.password = decodePapPassword(userPassword.getAttributeData(), sharedSecret.getBytes("UTF-8"));

				// copy truncated data
				userPassword.setAttributeData(password.getBytes("UTF-8"));
	 ...
	 * 
	 * @see com.doubleclue.portaldemo.radius.tinyradius.packet.RadiusPacket#encodeRequestAttributes(java.lang.String)
	 */
	protected void encodeRequestAttributes(String sharedSecret) {
		...
		if (getAuthProtocol().equals(AUTH_PAP)) {
			byte[] pass;
			try {
				pass = encodePapPassword(password.getBytes("UTF-8"), sharedSecret.getBytes("UTF-8"));
			} catch (UnsupportedEncodingException e) {
				pass = encodePapPassword(password.getBytes(), sharedSecret.getBytes());
			}
	...
	private String decodePapPassword(byte[] encryptedPass, byte[] sharedSecret) throws RadiusException {
		...
		// convert to string
		return getStringFromUtf8(passtrunc);
	}
