...
@Named("myDevicesView")
@SessionScoped
public class MyDevicesView extends DcemView {
...
// TODO		portalSessionBean.isActionEnable(ActionItem.NETWORK_DEVICE_DELETE_ACTION);
	...
	public void validateSelectedDevices() throws DcemException {
		if (selectedDevices != null && selectedDevices.isEmpty() == false) {
			...
			JsfUtils.addErrorMessage(resourceBundle.getString("message.noFilesSelected"));
		}
	}
	...
	public String getTranslatedState(AsApiDevice.StateEnum state) {
		if (state != null) {
			switch (state) {
			case Enabled:
				return resourceBundle.getString("label.enabled");
			case Disabled:
				return resourceBundle.getString("label.disabled");
			case TempLocked:
				return resourceBundle.getString("label.temp_locked");
			}
		}
		return "";
	}
...
// TODO			portalSessionBean.isActionEnable(ActionItem.OTP_DELETE_ACTION);
	...
	public String getDialogMessage(AsApiOtpToken token) {
		return resourceBundle.getString("dialog.message.removeToken") + " " + token.getSerialNumber();
	}
	...
	public String getDialogMessage(AsApiDevice device) {
		return resourceBundle.getString("dialog.message.deleteDevice") + " " + device.getName();
	}
