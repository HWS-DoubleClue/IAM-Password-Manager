...
import com.doubleclue.dcem.ps.subjects.ApplicationAdminSubject;
...
@Named("applicationAdminView")
@SessionScoped
public class ApplicationAdminView extends DcemView {

	private static final long serialVersionUID = 1L;

	@Inject
	private ApplicationAdminSubject applicationAdminSubject;
	...
	private Logger logger = LogManager.getLogger(ApplicationAdminView.class);

	@PostConstruct
	public void init() {
		subject = applicationAdminSubject;
		try {
	...
	public void handleFilesUpload(FileUploadEvent event) {
		...
		if (uploadedfile != null) {
			...
			if (i == -1 || "dcMyApp".equals(uploadedfile.getFileName().substring(i + 1)) == false) {
				JsfUtils.addErrorMessage(JsfUtils.getStringSafely(PasswordSafeModule.RESOURCE_NAME, "appHubAdmin.error.invalidFileExt"));
				return;
			}
			try {
			...
			JsfUtils.addErrorMessage(JsfUtils.getStringSafely(PasswordSafeModule.RESOURCE_NAME, "appHubAdmin.error.fileNotFound"));
			showDialog("importAppDlg");
		}
	}
	...
	public void actionImportApps() {
		if (uploadedFiles == null || uploadedFiles.isEmpty()) {
			JsfUtils.addErrorMessage(JsfUtils.getStringSafely(PasswordSafeModule.RESOURCE_NAME, "appHubAdmin.error.selectOneFileAtLeast"));
			return;
		}
		try {
			...
			JsfUtils.addErrorMessage(JsfUtils.getStringSafely(PasswordSafeModule.RESOURCE_NAME, "appHubAdmin.error.invalidFile"));
			showDialog("importAppDlg");
		}
	...
	public void actionAddApplication() {
		...
		try {
			...
			if (appNameValue != null && appNameValue.trim().isEmpty() == false && appNameValue.equals(currentApp.getName()) == false) {
				...
				JsfUtils.addErrorMessage(PasswordSafeModule.RESOURCE_NAME, "appHubAdmin.error.missingName");
				return;
			}
			if (appUrlValue.trim().isEmpty() || "http://".equals(appUrlValue)) {
				JsfUtils.addErrorMessage(PasswordSafeModule.RESOURCE_NAME, "appHubAdmin.error.missingUrl");
				return;
			} else {
				currentApp.getApplication().setUrl(appUrlValue);
			}
			if (currentApp.getApplication().getActions().isEmpty() == true) {
				JsfUtils.addErrorMessage(PasswordSafeModule.RESOURCE_NAME, "appHubAdmin.error.missingActions");
				return;
			}
			...
			if (currentApp.getLogo() != null) {
				try {
					...
					if (exp.getErrorCode() == DcemErrorCodes.IMAGE_TOO_BIG) {
						...
						JsfUtils.addErrorMessage(PasswordSafeModule.RESOURCE_NAME, "appHubAdmin.error.failedToResizeImg");
						return;
					}
				}
			}
			upAppHubLogic.updateApplication(currentApp);
			DcemUtils.reloadTaskNodes(PmAppHubLogic.class, TenantIdResolver.getCurrentTenantName());
	...
	public void actionAddEditAction() {
		...
		case delay:
			if (editedAction.getValueParameter() == null || getNumeric(editedAction.getValueParameter()) == -1) {
				JsfUtils.addErrorMessage(PasswordSafeModule.RESOURCE_NAME, "appHubAdmin.error.missingActionDelayValue");
			...
			if (getNumeric(editedAction.getValueParameter()) != -1) {
				...
				JsfUtils.addErrorMessage(PasswordSafeModule.RESOURCE_NAME, "appHubAdmin.error.numericValue");
				PrimeFaces.current().ajax().update("addAppsForm:tabView:actionsTable");
			}
		...
		case button:
			if (editedAction.getSelector() == null || editedAction.getSelector().isEmpty()) {
				JsfUtils.addErrorMessage(PasswordSafeModule.RESOURCE_NAME, "appHubAdmin.error.missingActionSelector");
				PrimeFaces.current().ajax().update("addAppsForm:tabView:actionsTable");
				return;
			}
		...
		case input:
			...
			if (isActionWithParameter() == true) {
				if (editedAction.getValueParameter() == null || editedAction.getValueParameter().isEmpty()) {
					JsfUtils.addErrorMessage(PasswordSafeModule.RESOURCE_NAME, "appHubAdmin.error.missingActionInputName");
					PrimeFaces.current().ajax().update("addAppsForm:tabView:actionsTable");
					return;
				}
			}
	...
	public void clearAction() {
		if (selectedActions == null || selectedActions.isEmpty()) {
			JsfUtils.addErrorMessage(PasswordSafeModule.RESOURCE_NAME, "appHubAdmin.error.selectAction");
			return;
		}
	...
	public void onEditAction() {
		...
		if (selectedActions.size() != 1) {
			JsfUtils.addErrorMessage(PasswordSafeModule.RESOURCE_NAME, "appHubAdmin.error.selectOnlyOneAction");
			return;
		}
	...
	public void actionFinishExecutePlugin() {
		try {
			...
			if (pluginResponse == null || pluginResponse.isEmpty()) {
				JsfUtils.addErrorMessage(PasswordSafeModule.RESOURCE_NAME, "error.PLUGIN_NO_RESPONSE");
			} else {
				ObjectMapper objectMapper = new ObjectMapper();
