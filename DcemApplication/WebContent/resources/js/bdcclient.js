...
function bdcLoc() {
	try{
			var type = $('#locForm\\:locationType').val();
				if (type == 'None') {
					return;
				}
				if (type == 'IP')	{
					 var client=new BDCApiClient("");
					    client.localityLanguage='en';
					    client.call(
					        'client-info',
					        {
					        },
					        function(jsonResult) {
					           $('#loginForm\\:location').val(jsonResult.ipString);
					        },
					        function(err,code) {
					...
					var apiKey= $('#locForm\\:locationApi').val().substring(2); 		
				    ...
				    client.call(
				        ...
				        function(jsonResult) {
				            ...
				            $('#loginForm\\:location').val(jsonResult.ip + '; ' + jsonResult.country.name + '; ' + jsonResult.location.postcode +  '; ' + jsonResult.location.city);
				            console.log('Location Info:',$('#loginForm\\:location').val());
				        },
				        function(err,code) {
				            console.log('Vanilla error',err,code);
				        }
