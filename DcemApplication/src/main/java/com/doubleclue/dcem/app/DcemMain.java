...
public class DcemMain {
	...
	public static void main(String[] args) {
		...
		try {
			File dcemHomeFile = LocalPaths.getDcemHomeDir();
			System.out.println("DCEM_HOME at " + dcemHomeFile.toString());
		...
		File webAppDir = null;
		try {
			tomcat.setBaseDir(LocalPaths.getDcemInstallDir().getAbsolutePath());
			String webAppName = clusterConfig.getWebAppName();
			if (webAppName.charAt(0) != '/') {
				webAppName = '/' + webAppName;
			}
			webAppDir = new File(LocalPaths.getDcemInstallDir(), webappDirLocation);
			standardContext = (StandardContext) tomcat.addWebapp(webAppName, webAppDir.getAbsolutePath());
		} catch (Exception exp) {
		...
		try {
			logger.info("WebAppDir: " + webAppDir.getAbsolutePath());
		...
		try {
			File additionWebInfClasses = new File(LocalPaths.getDcemInstallDir(), "bin");
		...
		try {
			...
			for (ConnectionService connectionService : clusterConfig.getConnectionServices()) {
				...
				case MANAGEMENT:
					addFilter(standardContext, LoginWebFilter.class.getSimpleName(), LoginWebFilter.class.getName(), DcemConstants.WEB_MGT_CONTEXT + "/*",
							enabled);
					break;
				case SAML:
				...
				if (enabled) {
					...
					case USER_PORTAL:
						...
						addServlet(standardContext, DcemConstants.LICENCE_SERVLET_NAME, DcemConstants.LICENCE_SERVLET_CLASS,
								DcemConstants.LICENCE_SERVLET_PATH);
						break;
		...
		String msg2 = "tomcat started in " + ((System.currentTimeMillis() - startTomcat) / 1000) + " seconds.";
