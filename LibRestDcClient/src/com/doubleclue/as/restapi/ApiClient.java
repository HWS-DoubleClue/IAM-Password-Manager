...
import com.doubleclue.as.restapi.auth.ApiKeyAuth;
import com.doubleclue.as.restapi.auth.Authentication;
import com.doubleclue.as.restapi.auth.HttpBasicAuth;
import com.doubleclue.as.restapi.auth.OAuth;
...
//	private int connectionTimeout = 0;
	...
	RestConnectionConfig connectionConfig;
	...
	public ApiClient(RestConnectionConfig connectionConfig2) {
...
//		this.dateFormat = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSZ");
		...
		authentications.put(HttpBasicAuth.class.getSimpleName(), new HttpBasicAuth());
	 ...
	 * @param config
	*/
	public void setConnectionConfig(RestConnectionConfig  config) {
		
		this.connectionConfig = config;
		// Rebuild HTTP Client according to the new "debugging" value.
		this.httpClient = buildHttpClient(connectionConfig.isDebug());
		httpClient.property(ClientProperties.CONNECT_TIMEOUT, connectionConfig.getConnectionTimeout());
		for (Authentication auth : authentications.values()) {
			if (auth instanceof HttpBasicAuth) {
				((HttpBasicAuth) auth).setUsername(connectionConfig.getOperatorName());
				((HttpBasicAuth) auth).setPassword(connectionConfig.getPassword());
				return;
			}
		}
		return;
	...
	*/
	...
	public <T> T invokeAPI(String path, String method, List<Pair> queryParams, Object body,
			...
			String[] authNames, GenericType<T> returnType) throws DcemApiException, ApiException {
		updateParamsForAuth(authNames, queryParams, headerParams);

		// Not using `.target(this.basePath).path(path)` below,
		// to support (constant) query string in `path`, e.g. "/posts?draft=1"
		WebTarget target = httpClient.target(connectionConfig.getRestApiUrl() + path);
...
//		if 
		// System.out.println("ApiClient.invokeAPI() " + response.getCookies().toString());
		...
		if (response.getStatus() == Status.NO_CONTENT.getStatusCode()) {
			 ...
			 * Emanuel Galea
			 */
		} else if (response.getStatus() == DcemApiException.SERVER_LOGIC_EXCEPTION) {
			GenericType<DcemApiException> exptionReturnType = new GenericType<DcemApiException>() {
			};
			DcemApiException apiException = (DcemApiException) deserialize(response, exptionReturnType);
			throw apiException;
