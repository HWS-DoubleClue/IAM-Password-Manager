...
public class LicenceLogic implements ReloadClassInterface {
	...
	private static final byte[] ENCRYPTION_KEY = { (byte) 0xAB, (byte) 0x32, (byte) 0x76, (byte) 0xFF, (byte) 0x3D,
			(byte) 0xB4, (byte) 0x77, (byte) 0x29, (byte) 0x0E, (byte) 0x82, (byte) 0xD6, (byte) 0xB3, (byte) 0xBB,
			(byte) 0x08, (byte) 0xC3, (byte) 0xD0 };
		...
		if (licenceKeyContent == null || userLogic.getTotalUserCount() >= licenceKeyContent.getMaxUsers()) {
			if (allowChanceAfterExpiration) {
				if (expiredLicenceUserShouldAuthenticate == true) {
					...
					throw new DcemException(DcemErrorCodes.LICENCE_MAX_USER,
							"Maximum user licence reached MaxUsers:  " + userLogic.getTotalUserCount());
				} else {
					expiredLicenceUserShouldAuthenticate = !expiredLicenceUserShouldAuthenticate;
				}
			}
	...
	public byte[] getEncryptedLicence(LicenceKeyContent licenceContent) throws DcemException {
		try {
			...
			throw new DcemException(DcemErrorCodes.INVALID_LICENCE_CONTENT,
					"Could not encrypt Licence Content: " + licenceContent);
		}
	}
	...
	public LicenceKeyContent loadLicenceKeyContent() throws DcemException {
		try {
			DcemConfiguration dcemConfiguration = configLogic.getDcemConfiguration(AdminModule.MODULE_ID,
					DcemConstants.CONFIG_KEY_LICENCE);
			AdminTenantData adminTenantData = adminModule.getAdminTenantData();
			if (dcemConfiguration == null) {
				LicenceKeyContent licenceKeyContent = createTrialLicence(DcemConstants.LICENCE_TRIAL_EXPIRY_DAYS,
						"Trial Licence");
	...
	public LicenceKeyContent createTrialLicence(int days, String customerName) throws DcemException {
		...
		return new LicenceKeyContent(clusterId, customerName, expiryDate, TenantIdResolver.getCurrentTenantName(),
				DcemConstants.LICENCE_KEY_VERSION, DcemConstants.LICENCE_PLUGINS_ALL);
	}
	...
	public void addLicenceToDb(LicenceKeyContent licenceContent) throws DcemException {
		...
		if (((tenantId == null || tenantId.isEmpty()) && TenantIdResolver.isCurrentTenantMaster())
			...
			try {
				...
				DcemConfiguration semConfiguration = configLogic.getDcemConfiguration(AdminModule.MODULE_ID,
						DcemConstants.CONFIG_KEY_LICENCE);
				if (semConfiguration == null) {
					semConfiguration = new DcemConfiguration(AdminModule.MODULE_ID, DcemConstants.CONFIG_KEY_LICENCE,
							encryptedLicence);
				} else {
				...
				throw new DcemException(DcemErrorCodes.INVALID_LICENCE_CONTENT,
						"Could not set licence: " + licenceContent);
			}
		} else {
			throw new DcemException(DcemErrorCodes.INVALID_LICENCE_CONTENT, "Wrong tenant while applying licence for: "
					+ tenantId + " Current Tenant: " + TenantIdResolver.getCurrentTenantName());
		}
	}
	...
	public List<DcemReporting> getLicenceWarnings() {
		...
		if (userCount >= maxUsers) {
			...
			if (remainingUsers < threshold) {
				alerts.add(createAlertMessage(getLocalisedWarning("maxUsersAlmostReached", remainingUsers),
			...
			if (admins != null && !admins.isEmpty()) {
				...
				for (DcemUser admin : admins) {
					...
					if (email != null && !email.isEmpty()) {
						...
						DcemTemplate bodyTemplate = templateLogic.getTemplateByNameLanguage(
								DcemConstants.EMAIL_LICENCE_WARNING_BODY_TEMPLATE, admin.getLanguage());
						if (bodyTemplate == null) {
							throw new DcemException(DcemErrorCodes.NO_TEMPLATE_FOUND,
									"Missing template: " + DcemConstants.EMAIL_LICENCE_WARNING_BODY_TEMPLATE);
						}
						...
						SendEmail.sendMessage(email, body,
								bundle.getString(DcemConstants.EMAIL_LICENCE_WARNING_SUBJECT_BUNDLE_KEY));
					}
				}
			}
		}
