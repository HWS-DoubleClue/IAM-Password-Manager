...
public class KeepassDatabaseWriterTest {
    ...
    public void shouldWriteDatabaseWithCustomIcon() throws IOException {
        ...
        byte[] customPng = Base64.getDecoder().decode(base64Icon);
        ...
        Assert.assertEquals(base64Icon, Base64.getDecoder().decode(readDb.getGroupByName("A").getIconData()));
        Assert.assertEquals(iconUuid, readDb.getEntryByTitle("1").getCustomIconUuid());
        Assert.assertEquals(base64Icon, Base64.getDecoder().decode(readDb.getEntryByTitle("1").getIconData()));
    }
