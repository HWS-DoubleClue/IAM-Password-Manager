package com.doubleclue.dcem.test.subjects;

import javax.enterprise.context.ApplicationScoped;

import com.doubleclue.dcem.core.DcemConstants;
import com.doubleclue.dcem.core.SubjectAbs;
import com.doubleclue.dcem.core.logic.RawAction;
import com.doubleclue.dcem.test.entities.TestLog;
import com.doubleclue.dcem.test.logic.TestModule;

@SuppressWarnings("serial")
@ApplicationScoped
public class LogSubject /*extends SubjectAbs*/ {

	public LogSubject() {

//		rawActions.add (new RawAction (DcemConstants.ACTION_CLEAR, new String [] {DcemConstants.SYSTEM_ROLE_SUPERADMIN, DcemConstants.SYSTEM_ROLE_ADMIN}));
//		rawActions.add (new RawAction (DcemConstants.ACTION_MANAGE, new String [] {DcemConstants.SYSTEM_ROLE_SUPERADMIN, DcemConstants.SYSTEM_ROLE_ADMIN}));
		
	}	

//	@Override
//	public String getModuleId() {
//		return TestModule.MODULE_ID;
//	}
//
//	@Override
//	public int getRank() {
//		return 40;
//	}
//
//	
//	@Override
//	public String getIconName() {
//		return "user_headset.png";
//	}
//
//	@Override
//	public String getPath() {
//		return DcemConstants.AUTO_VIEW_PATH;
//	}
//
//
//	@Override
//	public Class<?> getKlass() {
//		return TestLog.class;
//	}

}
